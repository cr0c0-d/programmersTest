...
'''
def solution(record) :
    # record의 길이가 10만 까지이므로  O(NlogN) 이하
    # 두 번째 단어는 항상 아이디임 => 아이디 찾을 때 사용할 수 있을지도
    # record를 split해서 2차원 배열로 사용?
    # 순회하며 아이디와 닉네임을 매핑해 딕셔너리로 저장
    answer = []
    ...
    return [id_dict[i[0]]+i[1] for i in answer]
