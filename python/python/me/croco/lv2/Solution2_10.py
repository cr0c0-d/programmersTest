...
def solution(data, col, row_begin, row_end) :
    ...
    for i in range(row_begin, row_end+1) :
        this_tuple = sorted_data[i-1]
        s_i_dict.append(sum(k % i for k in this_tuple))

    return reduce(lambda x, y : x ^ y, s_i_dict)
    # return reduce(lambda acc, i: acc ^ sum(k % i for k in sorted_data[i-1]), range(row_begin, row_end + 1), 0)    # 한 줄로 줄일면


data, col, row_begin, row_end = [[2,2,6],[1,5,10],[4,2,9],[3,8,3]], 2, 2, 3
